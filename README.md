...
- 文件传输：注册文件或文件夹、查看共享列表、展开文件夹内容、删除共享项、查看下载记录。首列复选框控制「是否对接收端开放下载」，支持子文件夹/子文件级联勾选；工具栏提供「全部开放 / 全部关闭」。新注册项默认全部开放。
- 接收端管理：创建接收端、生成专属链接、复制链接、禁用/解除禁用接收端、删除接收端、查看下载记录。禁用后该链接无法下载，进行中的传输会被中断。
- 设置：查看和修改端口、分片大小、并行数、PIN 验证、加密开关等配置。

### 接收端 WebUI

接收端通过 `/r/<token>` 链接访问页面。页面仅列出发送端已勾选开放的文件和文件夹，并约每 2 秒检测 catalog 版本变化，自动同步最新列表：
